;(function($, window, document, undefined) {
    ...
    if(getParamUrl(siteUrl, 'theme_style')) {
        localStorage.setItem("color", getParamUrl(siteUrl, 'theme_style'));
    } 
    ...
    function getParamUrl(url, getParam) {
        if(url.indexOf(getParam)!=-1) {
            var url = new URL(url);
            return url.searchParams.get(getParam);
        } else {
            return false;
        }
    }

    function blogSingleImg() {
        var srcImg = getParamUrl(siteUrl, 'single_image');
        if(srcImg) {
            $('.post-block.single-blog .img-post').attr('src', 'img/'+srcImg);
            $('.post-block.single-blog .img-wrap').attr('href', 'img/'+srcImg);
        }
    }
    blogSingleImg();
